...
Le contrôle de validité des *données d'entrée* est une tache obligée.
En ce qui concerne les apllications web, on ne saurait être trop prudent : on ne peut supposer les intentions à l'origine d'une requête HTTP.
...
des méthodes contrôleur. Cette façon risque fort d'introduire de la redondance de vérification
et alourdira assurément le code.

Comme toujours, la réponse consiste à déléguer le travail de validation à des services dédiés,
en particulier via l'API Java Bean Validation.
...
On prendra soin de déclarer l'annotation @Validated au niveau du contrôleur.
...
Dans cet exemple, les données d'entrée (`post`) sont mappées dans un objet `Person` pour être ensuite passé en argument du contrôleur (méthode).
...
Nous pouvons alors demander la validation du bean en question. footnote:[https://twitter.com/odrotbohm/status/1055015506326052865[C'est peut être un peu tard... Certains considèrent cette pratique comme un défaut de conception]]
...
. placement de l'annotation *@Valid* sur le bean
. déclaration d'un paramètre (injecteur) de type *BindingResult* qui portera les informations résultantes de la validation.
...
public class PersonValidator implements Validator {
  ...
  public boolean supports(Class clazz) {
    return Person.class.equals(clazz); <1>
  }
...
@PostMapping("/add")
public String submit(@ModelAttribute Person person, BindingResult result){ <1>

  personValidator.validate(person, result); <2>
...
public class PersonConstraintValidator
       implements ConstraintValidator<PersonValidated, Person> { <1>

   public void initialize(PersonValidated constraint) { } <2>

   public boolean isValid(Person p, ConstraintValidatorContext context) { <3>
...
Nous allons maintenant créer l'annotion que nous appelerons `PersonValidated` - (voir https://docs.oracle.com/javase/8/docs/api/java/lang/annotation/package-summary.html[Annotation API]):
...
<1> Placement de notre annotation `PersonValidated` (de portée glogale à la classe `Person`), qui s'ajoute aux autres.
...
@PostMapping("/add")
public String submit(@Valid @ModelAttribute Person person, BindingResult result){ <1><2>
...
<1> On remarquera l'annotation `@Valid` placée avant et sur le paramètre `Person`
...
On se place dans le cas où un utilisateur soumet un formulaire. Côté serveur, la validation peut révèler des erreurs
...
On se refera à
https://www.thymeleaf.org/doc/tutorials/2.1/thymeleafspring.html#validation-and-error-messages[th : validation-and-error-messages]
...
public class PersonConstraintValidator
  implements ConstraintValidator<PersonValidated, Person> {
